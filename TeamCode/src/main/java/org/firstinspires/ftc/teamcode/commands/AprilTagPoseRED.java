...
public class AprilTagPoseRED extends CommandBase {
    ...
    public AprilTagPoseRED(ScrappyAutoBase base, int id, boolean isBackCamera, double disAway, PoseVelocity2d offset) {
        m_base = base;
        m_desiredId = id;
...
//        avgY /= 3;
        ...
        m_action = m_base.robot.m_drive.actionBuilder(m_base.robot.m_drive.pose)
                .strafeToLinearHeading(AprilTagLocalization.getTagPosition(m_desiredId).plus(new Vector2d(m_desiredId <= 6 ? -m_disAway : m_disAway, 0)), Math.PI)
                .build();
